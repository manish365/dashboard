    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="theme-panel flex items-center justify-between rounded-xl border p-3 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg theme-tag-brand">
            <ShieldCheck className="theme-text-brand h-6 w-6" />
          </div>
          <div>
            <h2 className="theme-text text-sm font-bold">{title}</h2>
            <p className="theme-text-muted text-[10px] flex items-center gap-1">
              <span className="inline-block h-1.5 w-1.5 rounded-full theme-bg-success animate-pulse" />
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleRefresh} disabled={isRefreshing}
            ...
            className="theme-input theme-border flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all hover:bg-white/5 active:scale-95 disabled:opacity-50">
            <RefreshCw className={`h-3.5 w-3.5 ${isRefreshing ? 'animate-spin' : ''}`} />
            Refresh
          </button>
          <button suppressHydrationWarning className="theme-input theme-border flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all hover:bg-white/5 active:scale-95">
            <Maximize2 className="h-3.5 w-3.5" />
            Full Screen
          </button>
          <a href={ssoUrl} target="_blank" rel="noopener noreferrer"
            className="theme-btn-neon flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all hover:opacity-80 active:scale-95">
      ...
      <div className="theme-card-bg theme-border-header relative overflow-hidden rounded-2xl border shadow-2xl"
        ...
        {isLoading && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/60 backdrop-blur-md">
            <Loader2 className="theme-text-brand h-10 w-10 animate-spin" />
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between pointer-events-none">
          <div className="rounded-full theme-footer-bg border theme-border px-3 py-1 text-[9px] theme-text-subtle">
            CONFIDENTIAL · INTERNAL USE ONLY
          </div>
          <div className="rounded-full theme-tag-info border theme-border px-3 py-1 text-[9px] theme-text-info font-mono">
            ID: PBI-7742-STORE-2026
          </div>
        </div>
      </div>
    </div>
