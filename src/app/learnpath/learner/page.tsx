    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[...Array(6)].map((_, i) => <div key={i} className="theme-card-bg h-56 rounded-xl animate-pulse" />)}
        </div>
      ) : roadmaps.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {roadmaps.map(r => {
            ...
            const progress = enrollment ? Math.round((enrollment.completed / (enrollment.total || 1)) * 100) : 0;
            return (
              <div key={r.id} className="theme-card-bg rounded-xl border p-5 flex flex-col gap-3 transition-all hover:-translate-y-0.5">
                <div className="flex flex-wrap gap-1.5">
                  <LevelBadge level={r.level} />
                  <span className="theme-border theme-text-subtle text-xs px-2 py-0.5 rounded-full border">{r.category}</span>
                </div>
                <div>
                  <h3 className="theme-text font-semibold text-sm leading-snug">{r.title}</h3>
                  <p className="theme-text-muted text-xs mt-1 line-clamp-2">{r.description}</p>
                </div>
                ...
                {r.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {r.tags.slice(0, 3).map(tag => (
                      <span key={tag} className="theme-footer-bg theme-text-subtle text-xs px-1.5 py-0.5 rounded">{tag}</span>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-auto">
                  {isEnrolled ? (
                    ...
                    <button onClick={() => enroll(r.id)} disabled={enrolling === r.id}
                      className="theme-btn-neon w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition-all hover:opacity-90 disabled:opacity-50">
