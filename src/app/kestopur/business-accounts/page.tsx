...
export default function BusinessAccountsPage() {
  ...
  const cols = [
    { key: 'account', label: 'Account', render: (a: BizAccount) => <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold" style={{ background: 'rgba(96,165,250,0.1)', color: '#60a5fa' }}>{a.account_name?.[0] || 'B'}</div><div><p className="text-sm font-bold theme-text">{a.account_name}</p><p className="text-xs font-mono theme-text-subtle">{a.account_number}</p></div></div> },
    ...
    { key: 'financials', label: 'Financials', render: (a: BizAccount) => <div className="text-xs space-y-0.5"><p className="theme-text-muted">Limit: <span className="font-bold theme-text">₹{a.credit_limit?.toLocaleString() || 0}</span></p><p style={{ color: '#34d399' }}>Avail: ₹{a.available_credit?.toLocaleString() || 0}</p><p className="theme-text-subtle">Terms: {a.payment_terms_days || 0}d</p></div> },
